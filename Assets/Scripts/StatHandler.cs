...
namespace TTW.Combat
{
    public class StatHandler : MonoBehaviour
    {
        ...
        [SerializeField] float savedTime = 0f;
        ...
        private void Update()
        {
            ...
            if (statTimeStamp.Count > 0) savedTime = statTimeStamp.Peek();
        }
        ...
        public void ApplyBuff(StatChanges thisStatChange, int statAmount)
        {
            ...
            if (totalBuffsOfType < buffMax)
            {
                ...
                float totalTimeReduction = currentTimer;

                foreach (float timeSaved in statTimeStamp)
                {
                    totalTimeReduction = timerTotal - (totalTimeReduction + timeSaved);
                }

                statTimeStamp.Enqueue(totalTimeReduction);
                statChangeAmount.Enqueue(statAmount);

                if (currentTimer == 0)
                {
                    currentTimer = timerTotal;
                    statTimeStamp.Dequeue();
                }
        ...
        {
            if (statTimeStamp.Count > 0) currentTimer = statTimeStamp.Dequeue();
        }
