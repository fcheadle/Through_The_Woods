...
namespace TTW.Combat
{
    public class Fighter : MonoBehaviour
    {
        ...
        [SerializeField] Material deadMaterial = null;
        ...
        public void Remove()
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                if (combatController.actorInPosition[i] == this)
                {
                    ...
                    for (var j = i; j < 3; j++)
                    {
                        if (j + 1 < combatController.actorInPosition.Count)
                        {
                            combatController.actorInPosition[j] = combatController.actorInPosition[j + 1];
                        }
                    }
        ...
        public bool CheckAttack(EnemyTarget target, int abilityInPosition)
        {
            ...
            if (abilities[abilityInPosition].actorTargetingType == ActorTargetingType.melee)
            {
                ...
                {
                    print("enemy not in melee range");
                    return false;
                }
            }
