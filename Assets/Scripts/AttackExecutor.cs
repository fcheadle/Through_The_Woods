...
namespace TTW.Combat
{
    public class AttackExecutor : MonoBehaviour
    {
        ...
        AbilityEffectHandler abilityEffectHandler;
        List<AttackReceiver> splashTargets;

        public VFX startingVFX;
        ...
        private void Awake()
        {
            ...
            abilityEffectHandler = FindObjectOfType<AbilityEffectHandler>();
            splashTargets = new List<AttackReceiver>();
        ...
        private void PreAnimation(Ability ability, List<AttackReceiver> targets, Fighter actor, bool isLinkedAbility)
        {
            ...
            foreach (AttackReceiver target in finalTargetList)
            {
                ...
                else
                {
                    ...
                    GetComponent<AnimationHandler>().SaveAttackInfoDamage(finalDamage);
                }
        ...
        private void StartAnimations(Ability ability, Fighter actor, List<AttackReceiver> targets)
        {
            ...
            foreach (AttackReceiver splashTarget in splashTargets)
            {
            ...
            print("animation type: " + ability.actorAnimation);

            //if (targets[0] != this.GetComponent<AttackReceiver>())
            //{

            //if (ability.startingVFX != null)
            //{
            //    Vector3 yOffset = new Vector3(0f, 5f, 0f);
            //    VFX newVFX = Instantiate(ability.startingVFX);
            //    newVFX.transform.position = this.transform.position + yOffset;

            //    if (newVFX.GetComponent<FXLookAt>() != null)
            //    {
            //        newVFX.GetComponent<FXLookAt>().target = targets[0];
            //    }
            //}

            //if (ability.dimTheLights)
            //{
            //    abilityEffectHandler.DimTheLights();
            //}

            //if (ability.animationTimer > 0f)
            //{
            //    abilityEffectHandler.AnimationTimerStart(ability.animationTimer);
            //}

            if (ability.abilityEffectHandler != null)
            {
                AbilityEffectHandler newAbilityEffectHandler = Instantiate(ability.abilityEffectHandler);
                newAbilityEffectHandler.GetComponent<AbilityFX>().caster = this;

                foreach (AttackReceiver target in targets)
                {
                    newAbilityEffectHandler.GetComponent<AbilityFX>().AddTarget(target);
                }
            }


            //switch (ability.actorAnimation)
            //    {
            //        case ActorAnimationType.attackLight:
            //            {
            //                print("weapon attack light animation");
            //                GetComponent<AnimationHandler>().DoWeaponAttack();
            //                GameEvents.current.AnimationStart();
            //                break;
            //            }

            //        case ActorAnimationType.block:
            //            {
            //                print("guard animation");
            //                GetComponent<AnimationHandler>().Defend();
            //                break;
            //            }

            //        case ActorAnimationType.cast:
            //            {
            //                print("casting animation");
            //                GetComponent<AnimationHandler>().DoMagicalAttack();
            //                GameEvents.current.AnimationStart();
            //                break;
            //            }
            //        case ActorAnimationType.ranged:
            //            {
            //            print("casting animation");
            //            GetComponent<AnimationHandler>().DoRangedAttack();
            //            GameEvents.current.AnimationStart();
            //            break;
            //            }
            //    default:
            ...
            if (ability.actorAnimation == ActorAnimationType.block) return;
                ...
                GetComponent<AnimationHandler>().SaveAttackInfoTargets(ability, targets);
            //}

            

            GetComponent<StateMachine>().state = State.animating;
        ...
        {
            //print("targets check: " + targets.Count);
            ...
            //print("targets no. equals: " + finalTargetList.Count);
            ...
            foreach (AttackReceiver target in finalTargetList)
            {
                ...
                {
                    //final target becomes the old final targets protector
                    var oldFinalTarget = finalTarget;

                    finalTarget = oldFinalTarget.protector;

                    //oldFinalTarget.isProtected = false;
                    //finalTarget.isProtecting = false;
                }
            ...
            GetComponent<AttackIndicator>().ClearCells();
            StartAnimations(ability, enemy, targets);
            //print("animations started.");
        }

        private void StartAnimations(EnemyAttack ability, EnemyTarget enemyTarget, List<AttackReceiver> targets)
        {
            //When all three animation states have been accounted for, perform queued action and end attack
            GameEvents.current.AnimationStart();
